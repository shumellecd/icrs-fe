...
const MOCK_USERS_URL = 'http://localhost:1002/api/users';
const MOCK_ORDERS_URL = 'http://localhost:1003/api/orders';
...
test('users page: userList component surfaces a service error instead of crashing', async () => {
  const page = await browser.newPage();
  await page.goto(`${URL}/pages/users/index.html`, { waitUntil: 'networkidle' });
...
test('orders page: orderList component surfaces a service error instead of crashing', async () => {
  const page = await browser.newPage();
  await page.goto(`${URL}/pages/orders/index.html`, { waitUntil: 'networkidle' });
...
test('users & orders pages render real data through the BFF once the mock backend is up', async () => {
  mock = spawn('node', ['mock-api/server.js'], { stdio: 'ignore' });
  await waitForServer(MOCK_USERS_URL, 10000);
  await waitForServer(MOCK_ORDERS_URL, 10000);

  const usersPage = await browser.newPage();
  await usersPage.goto(`${URL}/pages/users/index.html`, { waitUntil: 'networkidle' });
  await usersPage.waitForSelector('.list-group-item');
  const userNames = await usersPage.locator('.list-group-item').allTextContents();
  assert.deepEqual(userNames, ['Ada Lovelace', 'Alan Turing', 'Grace Hopper']);
  await usersPage.close();

  const ordersPage = await browser.newPage();
  await ordersPage.goto(`${URL}/pages/orders/index.html`, { waitUntil: 'networkidle' });
  await ordersPage.waitForSelector('table tbody tr');
  const orderRows = await ordersPage.locator('table tbody tr').count();
  assert.equal(orderRows, 3);
...
test('status page: calls the real GitHub status API through the BFF without crashing', async () => {
  const page = await browser.newPage();
  const errors = [];
  page.on('pageerror', (err) => errors.push(err.message));

  await page.goto(`${URL}/pages/status/index.html`, { waitUntil: 'networkidle' });
  await page.waitForFunction(() => window.Alpine !== undefined);

  await page.waitForSelector('.badge:visible, .alert-danger:visible', { timeout: 15000 });
  assert.deepEqual(errors, []);
